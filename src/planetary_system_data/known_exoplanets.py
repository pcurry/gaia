...
# "Second, it’s also entirely possible for an exoplanet candidate to be a “false positive.” The signal in the data which 
# suggests the presence of a planet may also be caused by the star’s natural variability, or by some other object, or simply by 
# error in the astronomical instruments or observational methods. The scientific community doesn’t consider a candidate to be 
# “confirmed” until it’s been verified by a second set of observations using a different instrument... and even then, some 
# candidates have later been found to be in error."

# "All of this means that when developing an interstellar setting, it’s reasonable to limit which exoplanet candidates are to 
# be included, based on the scientific community’s current level of confidence in each detection. Meanwhile, it’s also 
# plausible to add additional exoplanets beyond those that are already known to exist, especially if these are specifically 
# designed to be difficult to detect (relatively low mass, long orbital period, and so on)."

# "For the rest of this section, we will discuss how to work with data about confirmed exoplanet candidates. The best and 
# most convenient source for this data is the NASA Exoplanet Archive, although references provided for a star by SIMBAD will 
# also discuss candidates."

# Orbital Parameters

# "Nearly every exoplanet candidate has defined orbital parameters defined. For example, the NASA Exoplanet Archive lists 
# the following parameters for most candidates:
...
# "This means that given the available observations, the most likely value for the planet’s orbital radius is 11.55 AU. 
# However, the actual value could be up to 0.86 AU lower, or 0.98 AU higher, than this value. When placing a candidate 
# exoplanet in a star system, feel free to vary the given parameters within the confidence interval."

# "The transit detection method makes it difficult to estimate an exoplanet's orbital eccentricity. The Exoplanet Archive 
# may give an eccentricity of 0, or have no listing at all. In these cases, generating a random eccentricity will be necessary."
...
# minimum mass. This is marked by the mass being given not simply as MP, but as the more complex formulation MP sin i, where 
# i is the (unknown) inclination of the exoplanet’s orbital plane to our line of sight.  This is because we can only infer 
# the exoplanet’s mass by the component of its primary star’s movement that is directly toward and away from us. If the 
# exoplanet’s orbital plane is closer to being face-on to us, it takes more planetary mass to generate the observed movement 
# of the primary star."

# "When dealing with exoplanet candidates whose true mass is unknown, it’s worth selecting a value for sin i, between 0 and 1 
# but with a strong preference for higher values. One way to generate a value at random might be to roll percentile dice twice 
# to generate values between 0 and 1, and taking the larger of the two. If there is more than one exoplanet candidate in the 
# target planetary system, the same value of sin i should be used for all of them."
...
# Feel free to accept these estimates as given, or you may prefer to use the procedures in Step Sixteen of the design sequence 
# to generate your own estimates."
...
# radii. If one of the ratios is significantly larger than the average, this indicates a “gap” into which at least one 
# undetected exoplanet might be placed.
